...
link: https://arxiv.org/abs/2210.16156v2
tldr: Linear CKA is not a good correlation measurement as spurious correlations may arise
note:
quality:
  - good
...
#paper
## Takeaways
- The results are mainly for linear CKA, for non-linear CKA (which is not so used) I didn't check it as the authors don't prioritise it
- CKA fails to capture translations of the data (i.e. moving the data changes the CKA coefficient, so it's sensitive to linear transformations)
- CKA geometry can be induced in the cost of the network without big performance degradation, trivialising a little the strength of the results CKA may provide
